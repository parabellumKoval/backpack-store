...
<span>
  ...
  @foreach($products as $product)
    <div>
      ...
      <p>Старая цена: <s>{{ \Settings::get('dress.store.currency.symbol') . $product['old_price'] }}</s></p>
      @endif
      <p>Цена: {{ \Settings::get('dress.store.currency.symbol') . $product['price'] }}</p>
      <p>Количество: {{ $product['amount'] }} шт</p>
      <p>Сумма: <strong>{{ \Settings::get('dress.store.currency.symbol') . ($product['price'] * $product['amount']) }}</strong></p>
    </div>
    <br>
  @endforeach

  @if(\Settings::get('dress.order.enable_bonus', false))
    <h4>Использовано бонусов: <strong>{{ \Settings::get('dress.store.currency.symbol') . $bonusesUsed }}</strong></h4>
  @endif
  ...
  <hr>
  <h4>Сумма заказа: <strong>{{ \Settings::get('dress.store.currency.symbol') . ($entry->price - $bonusesUsed) }}</strong></h4>
</span>