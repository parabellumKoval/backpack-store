...
@if ($crud->fieldTypeNotLoaded($field))
  ...
  @push('crud_fields_scripts')
      <script>
        ...
        function setVisibilityByType(type, el) {
          let wrapper = null

          if(type === 'overprice') {
            wrapper = el.querySelector('[data-target=overprice]');
          }else if(type === 'exchange') {
            wrapper = el.querySelector('[data-target=exchange_coff]');
          }
        ...
        function setVisibilityByTarget(target, el) {
          let wrapper = null
          let wrapper2 = null

          if(target === 'brand') {
            wrapper = el.querySelector('[data-target=brand-list]');
          }else if(target === 'code') {
        ...
        function setVisibilityOnElements(values) {
          var repeatable_fields_values = JSON.parse(values);

          $("[data-repeatable-holder=rules] .repeatable-element").each((index, el) => {
            const target = repeatable_fields_values[index].target
            setVisibilityByTarget(target, el)

            const type = repeatable_fields_values[index].type
            setVisibilityByType(type, el)
          })
        }
